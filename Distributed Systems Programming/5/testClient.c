...
void http_other(struct http_r* purl, int fd, char* req);
...
int main(int argc, char* argv[])
{
	...
	if (argc != 3)
	{
		fprintf(stderr, "Usage: %s Server head/get/other\n",
			argv[0]);
		exit(EXIT_FAILURE);
	}
...
//For full output: comment out from here onwards
	///* HTTP/1.1 */
